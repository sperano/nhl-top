...
### Component State Ownership Model ⭐

**CRITICAL ARCHITECTURE PRINCIPLE**: Components own their UI state. Global state only holds shared data.
...
- **Navigation**: Current tab, panel stack (shared across components)
...
✅ **Component State Advantages**:
- Persists across renders (doesn't get rebuilt)
- Isolated testing (test component without global state)
- Clear ownership (component controls its own state)
- No synchronization bugs (single source of truth)

❌ **Global State Problems** (old pattern, now removed):
- Duplication (same state in global and component)
- Sync bugs (keeping two copies in sync)
- Tight coupling (components depend on global state shape)
- Hard to test (need full AppState for unit tests)

#### AppState Structure

```rust
pub struct AppState {
    pub navigation: NavigationState,  // current_tab, panel_stack
...
**UiState is minimal** (as of Phase 8 completion):
```rust
pub struct UiState {
    pub scores: ScoresUiState,      // Only game_date (for data effects)
    pub settings: SettingsUiState,  // Settings tab state (not yet migrated)
}
```

Note: `ScoresUiState.game_date` is **not duplication** - it serves a different purpose:
- **Global `game_date`**: Tells background refresh which schedule to load
- **Component `game_date`**: What date the UI is currently viewing
...
- Sub-reducers: `reduce_navigation`, `reduce_panels`, `reduce_data_loading`
- Message forwarders: `reduce_scores`, `reduce_standings` (convert actions to component messages)

#### Message Dispatch Flow ⭐

The message flow enables component state updates:
...
│  - Effect processed by Runtime                       │
└──────────────────────────────────────────────────────┘
```

**Key Functions**:
- `key_to_action()` in `keys.rs`: Converts key events to actions
- `reduce()` in `reducer.rs`: Routes actions to appropriate handlers
- `Runtime::dispatch_component_message()`: Forwards messages to components
- `Component::update()`: Component handles its own state updates
...
Simple widgets implement `StandaloneWidget` (no `Send + Sync`, no `clone_box`):
```rust
...
**When to use which widget trait:**
- **StandaloneWidget**: Widgets rendered directly to buffer, not participating in Element tree
  - No thread-safety requirements (no `Send + Sync`)
  - No cloning requirements (no `clone_box`)
  - Used for simple, focused rendering components
  - Examples: GameBox, ScoreTable
- **ElementWidget**: Widgets that participate in the Element tree
  - Requires `Send + Sync` (thread-safe for async rendering)
  - Requires `clone_box()` (Element tree needs cloneable widgets)
  - Used for component widgets that integrate with the runtime
  - Examples: StatusBarWidget, BoxscorePanelWidget, DocumentElementWidget
...
The `document_nav.rs` module provides **reusable document navigation** that eliminates code duplication across components.

#### The Problem (Before Phase 7)

Components that use documents (DemoTab, StandingsTab) had duplicate navigation logic:
- Focus next/previous element
- Scroll viewport to keep focus visible
- Handle left/right navigation between Row children
- Update focus index and scroll offset

This led to:
- Code duplication (same logic in multiple components)
- Maintenance burden (fix bugs in multiple places)
- Inconsistent behavior (subtle differences between implementations)

#### The Solution: Generic Module

`src/tui/document_nav.rs` provides:

**1. DocumentNavState** - Embeddable state struct:
```rust
...
**2. DocumentNavMsg** - Shared message enum:
```rust
...
**3. handle_message()** - Generic navigation handler:
```rust
pub fn handle_message(
    msg: &DocumentNavMsg,
    state: &mut DocumentNavState,
) -> Option<Effect> {
    // Generic logic for all navigation operations
}
```

#### How to Use It

**Step 1: Embed DocumentNavState in Component State**
```rust
#[derive(Debug, Clone, Default)]
pub struct MyTabState {
    pub doc_nav: DocumentNavState,  // ← Embed the state
    // ... other component-specific state
}
```

**Step 2: Add Navigation Messages**
```rust
#[derive(Debug, Clone, PartialEq)]
pub enum MyTabMsg {
    DocNav(DocumentNavMsg),  // ← Wrap the generic messages
    UpdateViewportHeight(u16),
    // ... other component-specific messages
}
```

**Step 3: Handle Messages in Component**
```rust
impl Component for MyTab {
    fn update(&mut self, msg: Self::Message, state: &mut Self::State) -> Effect {
        match msg {
            ...
            MyTabMsg::UpdateViewportHeight(h) => {
...
**Step 4: Dispatch Messages from Keys**
```rust
fn handle_my_tab_keys(key: KeyEvent, state: &AppState) -> Option<Action> {
    match key.code {
        KeyCode::Up => Some(Action::ComponentMessage(
            ComponentMessage::new("my_tab", MyTabMsg::DocNav(DocumentNavMsg::FocusPrev))
        )),
        KeyCode::Down => Some(Action::ComponentMessage(
            ComponentMessage::new("my_tab", MyTabMsg::DocNav(DocumentNavMsg::FocusNext))
        )),
        // ... other keys
    }
}
...
✅ **No Code Duplication**: All navigation logic in one place
✅ **Consistent Behavior**: Same navigation UX across all document-based components
✅ **Easy to Test**: Test generic module once, reuse everywhere
✅ **Easy to Extend**: Add new navigation features in one place
✅ **Type Safe**: Rust's type system ensures correct usage

#### Real Example: StandingsTab

```rust
// State embeds DocumentNavState
#[derive(Debug, Clone, Default)]
pub struct StandingsTabState {
    pub view_mode: StandingsViewMode,
    pub doc_nav: DocumentNavState,  // ← Embedded
}

// Messages wrap DocumentNavMsg
#[derive(Debug, Clone, PartialEq)]
pub enum StandingsTabMsg {
    DocNav(DocumentNavMsg),  // ← Wrapped
    SetViewMode(StandingsViewMode),
    UpdateViewportHeight(u16),
}

// Update delegates to generic handler
...
impl Component for MyComponent {
    ...
    fn view(&self, props: &Self::Props, state: &Self::State) -> Element {
        // Build element tree from props and state
        vertical(vec![
            // ... elements
        ])
    }
...
#### When to Use Component State vs Global State
...
- ✅ Shared navigation (current tab, panel stack)
...
These principles emerged from the React system refactor (Phases 1-10, completed 2025-11-26):
...
- Reading component state in `key_to_action()` is OK (via helper functions)
...
- Use `ComponentMessage::new(component_id, msg)` to dispatch
- Effects can return `Effect::Action(Action::ComponentMessage(...))`

**3. Generic Over Specific**
- Avoid hardcoded component checks (like `if component == "standings"`)
- Extract shared patterns into reusable modules (like `document_nav.rs`)
- Use composition over trait-based inheritance
- Embedded structs > Traits for shared state (Rust idiom)
...
- Data loading updates component state via `Runtime::update_component_state()`
- Main concern of global reducers: data loading and shared navigation

**5. Avoid Infinite Loops**
- **NEVER** dispatch actions from the render loop
- Effects should eventually resolve to `Effect::None`
- Use state checks to prevent redundant dispatches
- Viewport height comes from `area.height` at render time, don't store and dispatch

**6. Embedded Structs for Shared Behavior**
...
- Delegate messages to embedded state handlers
...
fn test_key_to_action() {
    ...
    assert!(matches!(action, Some(Action::ComponentMessage(_))));
}
```
...
**Pattern: Data Loading Updates Component State**
```rust
// In data_loading reducer:
StandingsLoaded(Ok(standings)) => {
    state.data.standings = Some(Arc::new(standings.clone()));

    // Update component state with focusable metadata
...
**Pattern: Reading Component State in Keys**
```rust
// In keys.rs:
...
// Use in key_to_action:
if is_browse_mode_active(state, runtime) {
    // Handle browse mode keys
}
```

**Pattern: Component Returns Effect with New Action**
```rust
impl Component for MyComponent {
    fn update(&mut self, msg: Self::Message, state: &mut Self::State) -> Effect {
        match msg {
            MyComponentMsg::LoadData(id) => {
                ...
                // Return effect to trigger data load
...
**Pattern: Forwarding Reducer**
```rust
// In reducers/my_tab.rs:
pub fn reduce_my_tab(
    state: AppState,
    action: MyTabAction,
    runtime: &mut Runtime,
) -> Option<(AppState, Effect)> {
    // Convert action to component message
