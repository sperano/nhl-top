...
mod tests {
    ...
    fn test_settings_start_editing_log_level() {
        ...
        // log_level is a list setting, so it should open modal instead of text editing
        assert_eq!(new_state.ui.settings.modal_open, true);
        assert_eq!(new_state.ui.settings.editing, true);
        // Modal should be positioned at current value ("info" is index 2 in ["trace", "debug", "info", "warn", "error"])
        assert_eq!(new_state.ui.settings.modal_selected_index, 2);
    }
