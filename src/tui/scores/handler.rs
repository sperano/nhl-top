...
pub async fn handle_key(
    ...
    // Delegate to container if present and not in date/box navigation mode
    if let Some(ref mut container) = state.container {
        if !state.subtab_focused && !state.box_selection_active {
            return matches!(container.handle_input(key), InputResult::Handled | InputResult::Navigate(_));
        }
    }

    // Handle box/game selection navigation
    ...
    } else if state.subtab_focused {
        // Handle date navigation when subtab is focused (and not in box selection)
        match key.code {
            KeyCode::Left => {
                if state.selected_index == DATE_WINDOW_MIN_INDEX {
                    // At left edge: shift window left by 1 day
                    // selected_index stays at 0, game_date decrements
                    navigate_to_date(-1, shared_data, refresh_tx).await;
                } else {
                    // Within window: move selection left
            ...
            KeyCode::Right => {
                if state.selected_index == DATE_WINDOW_MAX_INDEX {
                    // At right edge: shift window right by 1 day
                    // selected_index stays at 4, game_date increments
                    navigate_to_date(1, shared_data, refresh_tx).await;
                } else {
                    // Within window: move selection right
                    let old_index = state.selected_index;
                    state.selected_index += 1;
                    navigate_within_window(old_index, state.selected_index, shared_data, refresh_tx).await;
                }
            ...
            KeyCode::Down => {
                // Enter box selection mode (if there are games)
                if state.grid_dimensions.0 > 0 && state.grid_dimensions.1 > 0 {
...
// ============================================================================
//
// This entire file represents the old scores handler implementation.
// Keep for reference when rebuilding scores functionality with Container widgets.
//
// Key functionality to rebuild:
// - Date window navigation (5-date sliding window)
// - Within-window vs edge-shifting navigation logic
// - Box/game selection and navigation
// - Boxscore view mode with scrolling
// - Player detail panel navigation
// - Boxscore save to file functionality
// - Comprehensive test suite for date navigation logic
//
// ============================================================================
